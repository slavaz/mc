                                        ...
                                        filegui_dialog_type_t dialog_type)
{
    ...
    case FILEGUI_DIALOG_MULTI_ITEM:
        dlg_height = 17 - (!verbose ? 7 : file_op_compute_totals ? 0 : 2);
        break;
...
file_progress_show_count (FileOpContext * ctx, size_t done, size_t total)
{
    ...
    ui = ctx->ui;
    if (file_op_compute_totals)
        g_snprintf (buffer, BUF_TINY, ("%s %zu/%zu"), _("Files processed:"), done, total);
    else
        g_snprintf (buffer, BUF_TINY, _("%s %zu"), _("Files processed:"), done);
                          ...
                          gboolean show_summary)
{
    ...
    if (file_op_compute_totals)
    {
        ...
        if (tctx->bps == 0)
            g_snprintf (buffer, BUF_TINY, _("%s %s %s"), _("Time:"), buffer2, buffer3);
        else
        {
            file_bps_prepare_for_show (buffer4, (long) tctx->bps);
            g_snprintf (buffer, BUF_TINY, _("%s %s %s (%s)"), _("Time:"), buffer2, buffer3, buffer4);
        }
    }
    else
    {
        if (tctx->bps == 0)
            g_snprintf (buffer, BUF_TINY, _("%s %s"), _("Time:"), buffer2);
        else
        {
            file_bps_prepare_for_show (buffer4, (long) tctx->bps);
            g_snprintf (buffer, BUF_TINY, _("%s %s (%s)"), _("Time:"), buffer2, buffer4);
        }
    }
    ...
    if (!file_op_compute_totals)
        g_snprintf (buffer, BUF_TINY, _(" %s %s "), _("Total:"), buffer2);
    else
    {
        size_trunc_len (buffer3, 5, ctx->progress_bytes, 0, panels_options.kilobyte_si);
        g_snprintf (buffer, BUF_TINY, _(" %s %s/%s "), _("Total:"), buffer2, buffer3);
    }
