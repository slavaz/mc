...
do_external_panelize (char *command)
{
    ...
    panelize_change_root (vfs_path_clone (current_panel->cwd_vpath));
...
do_panelize_cd (struct WPanel *panel)
{
    ...
    if (*(vfs_path_get_by_index (panelized_panel.root_vpath, 0)->path) == '\0')
        panelize_change_root (vfs_path_clone (current_panel->cwd_vpath));
 ...
 */
void
panelize_change_root (vfs_path_t * new_root)
{
    if (panelized_panel.root_vpath != NULL)
        vfs_path_free (panelized_panel.root_vpath);
    panelized_panel.root_vpath = new_root;
}
