...
show_free_space (WPanel * panel)
{
    ...
    static char *old_cwd = NULL, *tmp_path;
    ...
    if (old_cwd == NULL || strcmp (old_cwd, tmp_path) != 0)
    {
        ...
        init_my_statfs ();
        g_free (old_cwd);
        old_cwd = g_strdup (tmp_path);

        if (mc_realpath (tmp_path, rpath) == NULL)
        {
            g_free (tmp_path);
            return;
        }
...
{
    char *return_path;
    vfs_path_t *last_vpath = vfs_path_new ();
    vfs_path_element_t *path_element =
        vfs_path_element_clone (vfs_path_get_by_index (panel->cwd_vpath, -1));
                    ...
                    gpointer init_data, gpointer data)
{
    ...
    {
        char *tmp_path = vfs_path_to_str (p->cwd_vpath);
        if (ev->cfg != NULL)
...
maybe_cd (int move_up_dir)
{
    if (panels_options.navigate_with_arrows && (cmdline->buffer[0] == '\0'))
    {
        ...
        {
            vfs_path_t *vpath = vfs_path_from_str (selection (current_panel)->fname);
...
goto_parent_dir (WPanel * panel)
{
    ...
    else
    {
        ...
        {
            char *tmp_root = vfs_path_to_str (panelized_panel.root_vpath);
        ...
        {
            char *dname = g_strndup (fname, bname - fname);
...
goto_child_dir (WPanel * panel)
{
    ...
    {
        vfs_path_t *vpath = vfs_path_from_str (selection (panel)->fname);
        do_cd (vpath, cd_exact);
        vfs_path_free (vpath);
    }
}
...
do_enter_on_file_entry (file_entry * fe)
{
    ...
    {
        vfs_path_t *fname_vpath = vfs_path_from_str (fe->fname);
    ...
    if (!is_exe (fe->st.st_mode) || !if_link_is_exe (full_name_vpath, fe))
    {
...
_do_panel_cd (WPanel * panel, const vfs_path_t * new_dir_vpath, enum cd_enum cd_type)
{
    ...
    if (cd_type == cd_parse_command)
    {
        if (!strcmp (new_dir, "-"))
        {
            char *tmp_path = vfs_path_to_str (panel->lwd_vpath);
    ...
    if (*new_dir == '\0')
        ...
        vpath = vfs_path_from_str (new_dir);
    ...
    {
        char *tmp_path = vfs_path_to_str (panel->cwd_vpath);
        panel->count =
...
directory_history_next (WPanel * panel)
{
    ...
    if ((nextdir != NULL) && (_do_panel_cd (panel, data_vpath, cd_exact)))
...
directory_history_prev (WPanel * panel)
{
    ...
    if ((prevdir != NULL) && (_do_panel_cd (panel, data_vpath, cd_exact)))
...
panel_callback (Widget * w, widget_msg_t msg, int parm)
{
    ...
    case WIDGET_FOCUS:
        ...
        {
            char *cwd = vfs_path_to_str_flags (panel->cwd_vpath, 0, VPF_STRIP_PASSWORD);
...
remove_encoding_from_path (const vfs_path_t * vpath)
{
    ...
    {
        GIConv converter;
        vfs_path_element_t *path_element = vfs_path_get_by_index (vpath, indx);
...
panel_reload (WPanel * panel)
{
    ...
    if (panels_options.fast_reload && !stat (tmp_path, &current_stat)
