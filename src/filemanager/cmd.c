    ...
    {
        {
            ...
            {
                char *s_str = diff_two_paths (other_panel->cwd_vpath, s);
                vfs_path_free (s);
        ...
        {
            vfs_path_t *src_vpath = vfs_path_from_str (src);
...
ext_cmd (void)
{
    ...
    if (dir == 0)
    {
        buffer_vpath = vfs_path_build_filename (mc_config_get_data_path (), MC_FILEBIND_FILE, NULL);
...
edit_mc_menu_cmd (void)
{
    ...
    case 1:
        buffer_vpath = vfs_path_build_filename (mc_config_get_data_path (), MC_USERMENU_FILE, NULL);
        check_for_default (menufile_vpath, buffer_vpath);
        break;
...
edit_fhl_cmd (void)
{
    ...
    if (dir == 0)
    {
        buffer_vpath = vfs_path_build_filename (mc_config_get_path (), MC_FHL_INI_FILE, NULL);
...
single_dirsize_cmd (void)
{
    ...
    if (S_ISDIR (entry->st.st_mode) && strcmp (entry->fname, "..") != 0)
    {
        ...
        if (compute_dir_size
            (vfs_path_from_str_flags (entry->fname, VPF_NO_CANON), ui, compute_dir_size_update_ui,
...
dirsizes_cmd (void)
{
    ...
    for (i = 0; i < panel->count; i++)
                ...
                || !panel->dirs_marked) && strcmp (panel->dir.list[i].fname, "..") != 0)
        {
            ...
            if (compute_dir_size (vfs_path_from_str_flags (panel->dir.list[i].fname, VPF_NO_CANON),
...
save_setup_cmd (void)
{
    ...
    vpath = vfs_path_build_filename (mc_config_get_path (), MC_CONFIG_FILE, (char *) NULL);
