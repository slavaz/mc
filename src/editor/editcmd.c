...
edit_save_file (WEdit * edit, const vfs_path_t * filename_vpath)
{
    ...
    if (this_save_mode == EDIT_DO_BACKUP)
    {
        ...
        if (mc_rename (real_filename_vpath, tmp_vpath) == -1)
        {
...
edit_load_syntax_file (WEdit * edit)
{
    ...
    if (dir == 0)
    {
        ...
        user_syntax_file_vpath =
            vfs_path_build_filename (mc_config_get_data_path (), EDIT_SYNTAX_FILE, NULL);
...
edit_load_menu_file (WEdit * edit)
{
    ...
    case 1:
        buffer_vpath = vfs_path_build_filename (mc_config_get_data_path (), EDIT_HOME_MENU, NULL);
        check_for_default (menufile_vpath, buffer_vpath);
        break;
...
edit_set_filename (WEdit * edit, const vfs_path_t * name_vpath)
{
    ...
    if (name_vpath == NULL)
        tmp_vpath = vfs_path_from_str ("");
    else
        tmp_vpath = vfs_path_clone (name_vpath);

    first_name = vfs_path_get_by_index (tmp_vpath, 0)->path;
    vfs_path_free (edit->filename_vpath);
    edit->filename_vpath = tmp_vpath;

    if (edit->dir_vpath == NULL && !g_path_is_absolute (first_name))
...
edit_save_as_cmd (WEdit * edit)
{
    ...
    if (exp_vpath != NULL)
    {
        ...
        {
            int rv;
            if (vfs_path_cmp (edit->filename_vpath, exp_vpath))
            {
                int file;
...
edit_paste_from_X_buf_cmd (WEdit * edit)
{
    ...
    tmp = vfs_path_build_filename (mc_config_get_cache_path (), EDIT_CLIP_FILE, NULL);
...
edit_sort_cmd (WEdit * edit)
{
    ...
    {
        vfs_path_t *tmp_vpath =
            vfs_path_build_filename (mc_config_get_cache_path (), EDIT_TEMP_FILE, NULL);
