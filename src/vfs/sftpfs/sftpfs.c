...
typedef struct sftpfs_super_data_t
{
    int sock;
    char *cwdir;
    char *host;
...
sftpfs_opendir (struct vfs_class *me, const char *dirname)
{
    ...
    return (SUP->sftpfs_handle == NULL) ? NULL : super;
}
...
sftpfs_write (void *data, const char *buf, size_t nbyte)
{
    ...
    return libssh2_sftp_write (fh, buf, nbyte);
}
...
sftpfs_lseek (void *data, off_t offset, int whence)
{
    ...
    else
    {
        ...
        sftpfs_open (super->me, SUP->sftp_filename, SUP->sftp_open_flags, 0);
        fh = SUP->sftpfile_handle;
        libssh2_sftp_seek (fh, offset);
    }

    return (off_t) libssh2_sftp_tell64 (fh);
}
