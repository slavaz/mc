...
{
    {"^\\s*User\\s+(.*)$", NULL, STRING, NULL},
    {"^\\s*HostName\\s+(.*)$", NULL, STRING, NULL},
    {"^\\s*IdentitiesOnly\\s+(.*)$", NULL, BOOLEAN, NULL},
    {"^\\s*IdentityFile\\s+(.*)$", NULL, FILENAME, NULL},
    {"^\\s*Port\\s+(.*)$", NULL, INTEGER, NULL},
    {"^\\s*PasswordAuthentication\\s+(.*)$", NULL, BOOLEAN, NULL},
    {"^\\s*PubkeyAuthentication\\s+(.*)$", NULL, STRING, NULL},
    {NULL, NULL, 0, NULL}
};
/* *INDENT-ON* */
                                       ...
                                       const vfs_path_element_t * vpath_element, GError ** error)
{
    ...
    while (!feof (ssh_config_handler))
    {
        ...
        if (pattern_block_hit || host_block_hit)
        {
...
sftpfs_get_config_entity (const vfs_path_element_t * vpath_element, GError ** error)
{
    ...
    {
        if (!sftpfs_fill_config_entity_from_config
...
#define CONFIG_ENTITY_MEMBER_OFFSET(property) &((sftpfs_ssh_config_entity_t *) NULL)->property

void
sftpfs_init_config_variables_patterns (void)
{
    void *structure_offsets[] = {
        CONFIG_ENTITY_MEMBER_OFFSET (user),
        CONFIG_ENTITY_MEMBER_OFFSET (real_host),
        CONFIG_ENTITY_MEMBER_OFFSET (identities_only),
        CONFIG_ENTITY_MEMBER_OFFSET (identity_file),
        CONFIG_ENTITY_MEMBER_OFFSET (port),
        CONFIG_ENTITY_MEMBER_OFFSET (password_auth),
        CONFIG_ENTITY_MEMBER_OFFSET (pubkey_auth)
    };
...
#undef CONFIG_ENTITY_MEMBER_OFFSET
