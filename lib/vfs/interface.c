...
mc_def_ungetlocalcopy (const vfs_path_t * filename_vpath,
                       const vfs_path_t * local_vpath, int has_changed)
{
    int fdin = -1, fdout = -1;
    char *local = vfs_path_get_last_path_str (local_vpath);
...
mc_getlocalcopy (const vfs_path_t * pathname_vpath)
{
    ...
    if (vfs_path_element_valid (path_element))
    {
        result = path_element->class->getlocalcopy != NULL ?
            path_element->class->
            getlocalcopy (pathname_vpath) : mc_def_getlocalcopy (pathname_vpath);
        if (result == NULL)
            errno = vfs_ferrno (path_element->class);
    }
    return result;
                   ...
                   int has_changed)
{
    ...
    if (vfs_path_element_valid (path_element))
    {
        return_value = path_element->class->ungetlocalcopy != NULL ?
            path_element->class->ungetlocalcopy (pathname_vpath, local_vpath,
                                                 has_changed) :
...
mc_mkstemps (vfs_path_t ** pname_vpath, const char *prefix, const char *suffix)
{
    ...
    char *ret_path;
    int count;
    size_t XXXXXX_offset;

    tmpname = g_strconcat (prefix, "XXXXXX", suffix, (char *) NULL);
    if (strchr (prefix, PATH_SEP) == NULL)
    {
        /* Add prefix first to find the position of XXXXXX */
        ret_path = g_build_filename (mc_tmpdir (), tmpname, NULL);
        XXXXXX_offset = strlen (mc_tmpdir ()) + 1 + strlen (prefix);
    }
    else
    {
        ret_path = g_strdup (tmpname);
        XXXXXX_offset = strlen (prefix);
    }
    g_free (tmpname);

    XXXXXX = &ret_path[XXXXXX_offset];
    ...
    for (count = 0; count < TMP_MAX; ++count)
    {
        ...
        fd = open (ret_path, O_RDWR | O_CREAT | O_TRUNC | O_EXCL, S_IRUSR | S_IWUSR);
        if (fd >= 0)
        {
